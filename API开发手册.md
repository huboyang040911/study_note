...
允许智能体自主导航与检索还能实现**渐进式披露**（progressive disclosure）：每一步交互都会产生新的上 下文，反过来指导下一步决策——文件大小暗示复杂度、命名暗示用途、时间戳暗示相关性（当然模型能不能读懂这些暗示很大程度上取决于模型本身的智能程度）。

在实际上下文工程中，一般采用混合检索的策略：前置加载少量高价上下文，然后允许智能体按需自主调用工具探索。可以在项目的根目录放置一个“约定文件”（类似于Claude.md）。
