...
提示词工程关注的是提示词本身如何编写以获得更优的结果；而上下文工程则是关注一个**信息集合**，不仅包含提示词，还包含进入上下文窗口的所有信息。

在LLM落地的早期，那时主要针对单轮对话或者说某一个特定的任务做精调式的提示优化；当智能体出现时，这个任务过程中需要更多轮次，跨越更大的时间范围，因此需要管理整个上下文状态，包括System Prompt，ToolCalling，MCP，History Prompt等。
...
MCP指的是模型上下文协议，是一套标准化智能体与外部资源的通信方式，MCP制定统一的协议规范，让所**有服务以相同的方式访问**。
...
连接成功后查询服务器提供了哪些工具，调用list_tools()。
...
A2A的设计思想是**对等通信**，每个智能体可以发起请求也可以相应其他智能体的请求，这种设计避免了中心化协调器的瓶颈，让智能体网络更加灵活。
...
A2A是让智能体既能发请求，又能相应请求，所以我们需要创建A2A Agent服务器和客户端。
